...
void audioTask(void *pvParameters) {
    ...
    while (true) {
        ...
        if (playing && dataPtr != NULL && pos < len) {
            ...
            // Convert 8-bit unsigned PCM (0..255) to 16-bit signed PCM with 2.0x Gain Boost for Maximum Safe Volume
            for (uint32_t i = 0; i < chunkSize; i++) {
                int32_t sample = ((int32_t)rawBuf[i] - 128) * 512; // 2.0x maximum volume gain scaling
...
void loop() {
    ...
    if (Serial.available()) {
        ...
        line.trim();
        if (line == "PLAY:DEAUTH" || line.indexOf("\"deauth\"") != -1 || line == "ALARM_ON") playTrack("deauth");
        else if (line == "PLAY:EVILTWIN" || line.indexOf("\"evil_twin\"") != -1) playTrack("evil_twin");
        else if (line == "PLAY:SPOOFING" || line.indexOf("\"mac_spoof\"") != -1) playTrack("mac_spoof");
        else if (line == "PLAY:GREETING" || line.indexOf("\"greeting\"") != -1) playTrack("greeting");
        else if (line == "PLAY:UNACK" || line.indexOf("\"history\"") != -1) playTrack("history");
    }
